# ============================================================
# SECTION 4 (FULL SCRIPT - FIXED)
# Logistic regression - CN vs AD
# 5-fold CV on STABLE CN+AD (patient-level stratified splits)
#
# Predictors: functional_depth + age + GENOTYPE
# Depth is computed as mean MBD vs CN reference curves (TRAIN-only per fold)
#
# Outputs:
...
#
# Progress:
#  - VERY granular % progress for curve-building (train/test per fold)
# ============================================================
...
data_stable <- datatot %>%
  ...
  filter(DIAG %in% c("CN", "AD"))

# Converters: final diagnosis CN/AD only
data_conv <- datatot %>%
  filter(nav != 1) %>%
  group_by(RID) %>%
  filter(n_distinct(DIAGNOSIS) != 1) %>%
...
# ----------------------------
# 1) Patient-level labels + folds (stratified by class)
# ----------------------------
...
#    Returns list-of-matrices (one matrix per region)
# ----------------------------
...
calc_multi_depth <- function(mf_test, mf_ref) {
  if (mf_ref$N == 0) stop("mf_ref (CN reference) is empty. Cannot compute depths.")
...
# ----------------------------
# Helper: patient-level metadata (age/genotype) + attach depth by RID order
# ----------------------------
...
cat("Running Logistic regression (CN vs AD) with CV on stable patients...\n")
t0 <- Sys.time()
...
for (f in 1:K_FOLDS) {
  ...
  # CN reference patients in TRAIN
  cn_train_pat <- train_df %>% filter(DIAG == "CN") %>% distinct(RID) %>% pull(RID)
  
  if (length(cn_train_pat) == 0) {
    cat("WARNING: No CN patients in TRAIN fold -> cannot build CN reference. Skipping fold.\n")
    next
  }
  
  # ----------------------------
  # Build mfData for TRAIN (all train patients) + CN reference subset
  # ----------------------------
  cat("\nBuilding TRAIN mfData (for depth + CN reference)...\n"); flush.console()
  mf_train <- mfData(
    grid,
    create_mf_list_progress(train_sc, train_pat, all_volume_names, grid, label = "TRAIN")
  )
  
  mf_cn_ref <- mf_train[which(train_pat %in% cn_train_pat)]
  
  if (mf_cn_ref$N == 0) {
    cat("WARNING: CN reference mfData ended up empty. Skipping fold.\n")
    next
  }
  
  # Depth for TRAIN vs TRAIN-CN reference
  depth_train <- calc_multi_depth(mf_train, mf_cn_ref)
  ...
  # ----------------------------
  # Build mfData for TEST and compute depth vs TRAIN CN reference
  # ----------------------------
  cat("\nBuilding TEST mfData (depth vs TRAIN CN reference)...\n"); flush.console()
  mf_test <- mfData(
    grid,
    create_mf_list_progress(test_sc, test_pat, all_volume_names, grid, label = "TEST ")
  )
  
  depth_test <- calc_multi_depth(mf_test, mf_cn_ref)
  ...
  # Align GENOTYPE factor levels safely
  ...
  # Drop rows that become NA after level alignment or missing predictors
  test_meta <- test_meta %>%
    filter(!is.na(GENOTYPE), !is.na(functional_depth), !is.na(age), !is.na(Outcome))
  
  if (nrow(test_meta) == 0) {
    cat("WARNING: test_meta is empty after cleaning/alignment. Skipping fold.\n")
    next
  }
  ...
  pred <- ifelse(prob > 0.5, "AD", "CN")
  true <- ifelse(test_meta$Outcome == 1, "AD", "CN")
  
  # ROBUST APPEND (avoid factor weirdness)
...
if (length(all_true) == 0 || length(all_pred) == 0) {
  ...
  cat("This usually means folds were skipped due to empty CN reference or empty test_meta.\n")
} else {
...
# ============================================================
# 5) Converters accuracy
# Train on ALL stable CN/AD, test on converters CN/AD (final diagnosis)
# ============================================================
cat("\n==================== RESULTS: CONVERTERS ====================\n")
...
# Scaling from ALL stable (no leakage wrt converters evaluation)
...
cn_pat_all <- stable_all %>% filter(DIAG == "CN") %>% distinct(RID) %>% pull(RID)

if (length(cn_pat_all) == 0) {
  cat("ERROR: No CN patients in ALL stable. Cannot evaluate converters.\n")
} else {
  
  cat("Building ALL-STABLE mfData (CN reference from ALL stable CN)...\n"); flush.console()
  mf_stable_all <- mfData(
    grid,
    create_mf_list_progress(stable_sc, stable_pat_all, all_volume_names, grid, label = "ALLST")
  )
  
  mf_cn_all <- mf_stable_all[which(stable_pat_all %in% cn_pat_all)]
  if (mf_cn_all$N == 0) {
    cat("ERROR: CN reference mfData for ALL stable is empty.\n")
  } else {
    
    depth_stable_all <- calc_multi_depth(mf_stable_all, mf_cn_all)
    ...
    stable_meta_all <- stable_meta_all %>%
      ...
      mutate(Outcome = if_else(DIAG == "AD", 1, 0)) %>%
      filter(!is.na(functional_depth), !is.na(age), !is.na(GENOTYPE))
    
    cat("Building CONVERTERS mfData (depth vs ALL-stable CN reference)...\n"); flush.console()
    mf_conv <- mfData(
      grid,
      create_mf_list_progress(conv_sc, conv_pat_all, all_volume_names, grid, label = "CONV ")
    )
    
    depth_conv <- calc_multi_depth(mf_conv, mf_cn_all)
    ...
    if (nrow(conv_meta) == 0) {
      cat("ERROR: conv_meta became empty after GENOTYPE alignment.\n")
    } else {
      ...
      pred_c <- ifelse(prob_c > 0.5, "AD", "CN")
      true_c <- ifelse(conv_meta$Outcome == 1, "AD", "CN")
